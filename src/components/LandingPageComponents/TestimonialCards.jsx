        ...
        <div className='max-w-[1234px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-items-center place-items-center gap-10'>
            {
                testimonials.map((testimonial) => (
                    <div key={testimonial.id} className='w-full h-full lg:h-[465px] border border-gray rounded-[5px] flex flex-col justify-center items-start'>
                        <div onMouseOver={() => setTestimonialId(testimonial.id) } onMouseLeave={() => setTestimonialId(null)} className='relative w-full h-[352px]'>
                            <div className='bg-main rounded-tl-lg rounded-tr-lg absolute z-[5px] h-full lg:h-[331px] w-full'></div>
